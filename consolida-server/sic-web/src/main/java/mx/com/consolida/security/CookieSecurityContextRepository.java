...
public class CookieSecurityContextRepository implements SecurityContextRepository {
	...
	private Cookie cookieRemove(){
		Cookie userNameCookieRemove = new Cookie("userInfo","");
		return userNameCookieRemove;
	}
	...
	private Optional<UserDetailNomina> readUserInfoFromCookie(HttpServletRequest request) {
		return readCookieFromRequest(request).map(this::createUserInfo);
	}
	...
	private UserDetailNomina createUserInfo(Cookie cookie) {
	...
	private class SaveToCookieResponseWrapper extends SaveContextOnUpdateOrErrorResponseWrapper {
		...
		protected void saveContext(SecurityContext securityContext) {
			...
			if (!(authentication.getPrincipal() instanceof UserDetailNomina)) {
				LOG.warn("securityContext.authentication.principal of unexpected type {}, skip saveContext",
						authentication.getPrincipal().getClass().getCanonicalName());
				return;
			}

			UserDetailNomina userInfo = (UserDetailNomina) authentication.getPrincipal();
			SignedUserInfoCookie cookie = new SignedUserInfoCookie(userInfo, cookieHmacKey);
			cookie.setSecure(request.isSecure());
			cookie.setPath(cookie.getPath().concat("; SameSite=Strict;"));
			response.addCookie(cookieRemove());
