...
public class SignedUserInfoCookie extends Cookie {
	protected Logger logger = LoggerFactory.getLogger(SignedUserInfoCookie.class);
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 4573754066432330438L;
	public static final String NAME = "UserInfo";
	private static final String PATH = "/";
	private static final Pattern UID_PATTERN = Pattern.compile("uid=([A-Za-z0-9]*@(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9])");
	private static final Pattern ROLES_PATTERN = Pattern.compile("roles=([A-Z0-9_|]*)");
	private static final Pattern COLOUR_PATTERN = Pattern.compile("details=(.*)&");
	...
	public SignedUserInfoCookie(UserDetailNomina userInfo, String cookieHmacKey) {
		super(NAME, "");
	...
	public SignedUserInfoCookie(Cookie cookie, String cookieHmacKey) {
		...
		String calculatedHmac =calculateHmac(payload, cookieHmacKey);
		if (!hmac.equals(calculatedHmac)) {
			logger.debug("HMAC {} {} {}",payload,hmac,calculatedHmac);
	...
	public String getValue() {
		logger.debug("Cookie value {}",payload.toString());
		return payload.toString() + "&hmac=" + hmac;
	}

	public UserDetailNomina getUserInfo() {
		return UserDetailNomina.fromJson(payload.colour);
	}
	...
	private static class Payload {
		...
		public String toString() {
			return "uid=" + username + "&roles=" + String.join("|", roles)
					+ (colour != null ? "&details=" + colour : "");
		}
	}
